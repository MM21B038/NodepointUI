...
const Stream: React.FC<StreamProps> = () => {
  ...
  const [thinkingLogs, setThinkingLogs] = useState<any[]>([]); // Changed back to array
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (currentWorkspace) {
        ...
        setThinkingLogs([]); // Clear thinking logs
        setFinalAnswer(null);
      }
    };
    fetchData();
  ...
  const handleStreamSend = useCallback(async () => {
    ...
    setIsStreaming(true);
    setThinkingLogs([]); // Clear previous thinking logs for a new query
    setFinalAnswer(null);
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (event.step.startsWith("THINKING")) {
              setThinkingLogs(prev => [...prev, event]); // Append new thinking log
            }
              ...
              <AccordionItem value="thinking-panel" className="border-none">
                ...
                <AccordionContent className="pt-2 pb-0">
                  <div className="space-y-2 p-3 border rounded-lg bg-secondary/50 text-sm">
                    {thinkingLogs.length === 0 && !isStreaming && !finalAnswer ? (
