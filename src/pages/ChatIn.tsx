...
import { MessageCircle, Info, Loader2, Sparkles, Globe, FolderSearch, Zap, FileText, Send, Tag, X, Bot, User, ChevronDown, ChevronUp } from "lucide-react";
...
const ChatIn: React.FC<ChatInProps> = ({
  ...
  const scrollAreaViewportRef = useRef<HTMLDivElement>(null); // Ref for the ScrollArea's viewport
  ...
  const [showScrollToBottom, setShowScrollToBottom] = useState(false); // New state for scroll button visibility
  ...
  // Effect to scroll to bottom when messages change or history loads
  useEffect(() => {
    if (isLoadingHistory) return;
    // Only scroll to bottom if already near bottom or if it's the initial load
    const viewport = scrollAreaViewportRef.current;
    if (viewport) {
      const isAtBottom = viewport.scrollHeight - viewport.scrollTop - viewport.clientHeight < 100; // Threshold of 100px
      if (isAtBottom || messages.length <= 2) { // Scroll on initial messages or if user is near bottom
        scrollToBottom();
      }
    }
  }, [messages, isLoadingHistory, scrollToBottom]);

  // Effect to handle scroll detection for the "scroll to bottom" button
  useEffect(() => {
    const viewport = scrollAreaViewportRef.current;
    if (!viewport) return;
  ...
  }, [currentWorkspace, messages]); // Re-run if workspace or messages change
  ...
  }, [currentWorkspace, selectedFiles, scrollToBottom]);
      ...
      {!currentWorkspace ? (
        ...
        <div className="flex flex-col flex-grow mt-4 p-4 space-y-4 border-x-4 border-y-2 rounded-lg">
          <div className="flex-grow flex flex-col border rounded-lg shadow-sm z-[0]">
            <ScrollArea className="flex-grow h-0 w-full !transform-none hide-scrollbar" viewportRef={scrollAreaViewportRef}> {/* Changed to scrollAreaViewportRef */}
              <div className={cn(
